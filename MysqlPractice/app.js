...
app.get("/create", (req, res) => {
  ...
  Connection.query(createProducts, (err) => {
    if (err) return res.status(500).send("Error creating Products table");

    Connection.query(createProductsDescription, (err) => {
      if (err)
        return res.status(500).send("Error creating ProductsDescription table");

      Connection.query(createProductsPrice, (err) => {
        if (err)
          return res.status(500).send("Error creating ProductsPrice table");
        ...
        Connection.query(createUsers, (err) => {
          ...
          Connection.query(createOrders, (err) => {
            if (err) return res.status(500).send("Error creating Orders table");
...
app.put("/update", (req, res) => {
  ...
  const updateName = `UPDATE Products SET product_name = ? WHERE product_id = ?`;
...
// Get all products with their descriptions and prices
app.get("/products", (req, res) => {
  const query = `
    SELECT 
      ...
      pd.product_link
    FROM Products p
    JOIN ProductsDescription pd ON p.product_id = pd.product_id
    JOIN ProductsPrice pp ON p.product_id = pp.product_id
  `;
  ...
  Connection.query(query, (err, results) => {
    ...
    console.log("✅ Products fetched and sent to client:");
    console.table(results); // 👈 Logs in a clean table format in your terminal
